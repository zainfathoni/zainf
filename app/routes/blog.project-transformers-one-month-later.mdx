...
The idea for deterministic dispatch came from studying how others were solving this. OpenAI's [Symphony](https://github.com/openai/symphony) framework, [Paperclip](https://paperclip.ing), and especially Elvis's proactive task sourcing pattern — where his orchestrator "Zoe" scans Sentry errors and meeting notes to [autonomously spawn agents](https://x.com/elvissun/status/2028671336219107687) — all pointed in the same direction: **spend tokens on judgment, spend code on mechanics.**
...
1. **Queries [Fizzy](https://fizzy.do)** — pulls all cards tagged `bot-actionable` that haven't been worked on
...
- **amp-swarm** — takes multiple GitHub issues, groups them by conflict safety (issues that modify the same files can't run in parallel), spawns one [Amp](https://ampcode.com) deep-mode worker per issue. Each creates a branch, implements the fix, and opens a PR.
- **claude-swarm** — same concept, but using [Claude Code](https://docs.anthropic.com/en/docs/claude-code) (`claude --print`). Fallback when Amp credits are spent, or for repos that need deeper reasoning.
...
But lots of PRs without good reviews = technical debt factory. So we built a multi-reviewer pipeline — inspired by Elvis's triple AI review setup (Codex + Gemini + Claude) from [his article](https://x.com/elvissun/status/2025920521871716562), and the verification gates pattern from [Ryan Carson's Antfarm](https://x.com/ryancarson/status/2020931274219594107) where agents systematically check each other's work:
...
In Part 1, I talked about knowledge management — JSON files with structured facts, a memory pipeline, weekly extraction. That was the foundation. A month later, we built something more powerful on top of it: **compound engineering** — a concept borrowed from [Every.to's guide on the topic](https://every.to/guides/compound-engineering).
...
One upgrade that isn't flashy but was *crucial*: we now have dashboards. [Ryan Carson's suggestion](https://x.com/ryancarson/status/2033913074772586888) to spend a full day optimizing your agentic setup hit home — and observability was the first thing we built.
...
This instinct — fix the process, not the person — comes from [my time as a manager](https://zainf.dev/karir-it). I've managed teams twice: once leading ~70 people at a Singapore-based company, and later a team of 20+ handling the largest project in the company. The blameless culture I experienced there rewired how I think about failure. When Optimus skipped cards for 20 days, my reaction wasn't "this bot is dumb." It was "the system made it too easy to skip." The fix wasn't scolding the agent — it was building a deterministic dispatcher that removed the option to defer. That's a manager's reflex: when something goes wrong, you don't blame the individual — you fix the process that allowed it to happen.
...
[Elvis Sun](https://x.com/elvissun) — someone I respect in the AI tooling space — had [published his own setup](https://x.com/elvissun/status/2025920521871716562) around the same time. His orchestrator "Zoe" spawns Codex and Claude Code agents in tmux worktrees, monitors them via cron, and runs triple AI code review. Not an exact copy of our system, but the same principles: specialized agents, domain separation, knowledge persistence. He later shared a [follow-up on token optimization](https://x.com/elvissun/status/2028671336219107687) that directly influenced our dispatcher design.
